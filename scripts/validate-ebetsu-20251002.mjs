...
async function main() {
  ...
  assert.deepEqual(siteStructured, structured, "root/site structured data must stay synchronized");
  ...
  assert.equal(
    structured.source_document.extractor_version,
    "ebetsu-structured-minutes-v3-2026-08-11"
  );
  ...
  for (const [title, expected] of SINGLE_PARENT_TOPIC_CASES) {
    ...
    assert.equal(topic.public_visible, true, `single-parent topic must remain public: ${title}`);
  ...
  const visibleTopicIds = new Set(
    structured.topic_blocks.filter((topic) => topic.public_visible).map((topic) => topic.id)
  );
  ...
  assert.ok(
    ...
    `public topic snippet duplicate rate regressed: ${duplicateReferenceRate.toFixed(3)}`
  );
