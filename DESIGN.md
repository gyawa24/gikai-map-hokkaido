...
### 議事録の一覧・本文（2026-09-07）

- 一覧は会議名・開催期間・日程または資料の収録件数を一つの行にまとめ、年と会議種別で区切る。各会議を大きな装飾カードにしない。PDF一つに複数開催日を含む場合があるため、保存単位を開催日数とみなさない。
- 通常の本文ページは `MinutesHeading` を共用し、`max-w-3xl` の読みやすい幅で開催期間・収録範囲・原典導線を表示する。ローカル読み込みと遠隔読み込みで配置を変えない。
- 日程の選択表示、表示中の日程見出し、本文、引用は必ず同じ有効日程に揃える。検索後も利用者が明示的に日程を選べるようにする。
- 原典は `MinutesSourceLink` を共用する。個別資料のURLと公式一覧・検索へのURLを区別し、不明なものを個別原典として表示しない。
- 日付の精度は `minutesDateLabel` を共用する。月までの記録に架空の日付を補わず、不明は「開催日未確認」と表示する。
- 読み込み障害は「未掲載」と区別し、再読み込みと公式資料への導線を出す。AI要約と自動整理情報は本文の原文と区別する。
