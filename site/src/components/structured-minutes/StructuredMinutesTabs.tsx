...
export default function StructuredMinutesTabs({
  ...
  const topicsWithheld = view === "topics" && publicTopicBlocks.length === 0 && data.topic_blocks.length > 0;
  ...
  return (
    <section>
      ...
      {topicsWithheld ? (
        <div role="status" className="mb-4 rounded-lg border border-[#E2E8F0] bg-white px-4 py-4 text-sm leading-relaxed text-[#4A5568]">
          <p className="font-bold text-[#1A202C]">質問項目別の表示は確認待ちです</p>
          <p className="mt-1">自動整理した項目には確認記録が揃っていません。発言本文や質問者ごとのまとまりから内容を確認できます。</p>
          <div className="mt-3 flex flex-wrap gap-4">
            <Link href={tabHref(basePath, "turns", 1, query)} className="font-bold text-[#1B3A6B] underline">発言単位で読む</Link>
            <Link href={tabHref(basePath, "questions", 1, query)} className="font-bold text-[#1B3A6B] underline">質問者別で読む</Link>
          </div>
        </div>
      ...
      {!topicsWithheld && <div className="mb-4 flex flex-col gap-3 rounded-lg border border-[#E2E8F0] bg-white px-4 py-3 text-sm text-[#4A5568] sm:flex-row sm:items-center sm:justify-between">
        <p>
          {query ? (
            <>
      ...
      {query && activeItemCount === 0 && !topicsWithheld && (
        <div
          role="status"
