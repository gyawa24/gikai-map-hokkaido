...
export default function ResearchAccessGate({ configured }: Props) {
    ...
    return (
      <section className="theme-alert px-5 py-5 text-[#78451F]" role="alert">
        ...
        <p className="mt-2 text-sm leading-relaxed">
          管理者がアクセス設定を完了するまで、この機能は利用できません。
        </p>
      </section>
    );
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(responseMessage(payload, "ログインできませんでした。少し待ってから再度お試しください。"));
        return;
      }
  ...
  return (
    <section className="theme-panel mx-auto max-w-xl px-5 py-6 sm:px-7" aria-labelledby="research-login-title">
      <h2 id="research-login-title" className="theme-section-title text-xl sm:text-2xl">
        限定公開ページ
      </h2>
      <p className="mt-3 text-sm leading-relaxed text-[#4A5568]">
        政策AIリサーチャーは現在検証中です。共有されたパスワードを入力してください。
      </p>
      <form className="mt-5" onSubmit={handleSubmit}>
        <label htmlFor="research-access-password" className="block font-bold text-[#1B3A6B]">
          パスワード
        </label>
        <input
