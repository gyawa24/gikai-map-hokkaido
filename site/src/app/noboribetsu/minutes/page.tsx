...
function getEnriched(councilId: number): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "noboribetsu", "minutes", "enriched", `${councilId}.json`);
  try {
...
export default function NoboribetsuMinutesPage() {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <section className="mb-6">
        ...
        <p className="text-base text-[#4A5568] leading-relaxed">
          登別市議会の公式会議録です。本会議・委員会の発言内容をすべて収録しています。
          {items.length > 0 && (
      ...
      {items.length === 0 ? (
        ...
        <MinutesIndexClient items={items} minutesBasePath="/noboribetsu/minutes" />
      )}
    </div>
  );
}
