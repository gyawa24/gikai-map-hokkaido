...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "hakodate", "members.json");
...
export default function HakodateMembersPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("hakodate");
    ...
    return (
      <>
        ...
        <section className="mb-6">
          ...
          <p className="text-base text-[#4A5568] leading-relaxed">函館市議会の議員情報です。</p>
        </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/hakodate/members" />
    </>
  );
}
