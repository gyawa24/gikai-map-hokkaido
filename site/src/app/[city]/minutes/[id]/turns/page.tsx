...
import { getMinutesCatalogSource, safeMinutesSourceUrl } from "@/lib/minutesSource";
import { buildPageMetadata } from "@/lib/metadata";
import { getPublishedMinutesIndexItem, getPublishedMinutesIndexResult } from "@/lib/minutesPublication";
import { getStructuredMinutesResult } from "@/lib/structured-minutes/loadStructuredMinutes";
...
export default async function StructuredMinutesPage({ params, searchParams }: Props) {
  ...
  const result = publication.status === "available"
    ? await getStructuredMinutesResult(city, id)
    : { status: publication.status };
  if (result.status === "absent") notFound();
  if (result.status !== "available" || !("data" in result)) {
    return (
      <div className="page-shell max-w-6xl">
        <h1 className="text-2xl font-bold text-[#1B3A6B]">構造化議事録を確認できませんでした</h1>
        <p className="mt-3 text-[#4A5568]">読み込みまたはデータの整合性を確認できませんでした。時間をおいて再度お試しください。</p>
        <div className="mt-5 flex gap-4">
          <a href={`/${city}/minutes/${id}/turns`} className="font-bold text-[#1B3A6B] underline">再読み込み</a>
          <Link href={`/${city}/minutes/${id}`} className="font-bold text-[#1B3A6B] underline">議事録詳細・公式会議録を確認</Link>
        </div>
      </div>
    );
  }
  const data = result.data;
  const catalogSource = getMinutesCatalogSource(municipality);
  const officialUrl = safeMinutesSourceUrl(data.source_document.official_url);
  const sourceLabel = officialUrl && (
    officialUrl === catalogSource?.url
    || /\/voices\/?(?:\?.*)?$/u.test(officialUrl)
    || /\/MinuteBrowse\.html$/u.test(officialUrl)
  ) ? "公式会議録の一覧・検索" : "公式掲載ページを確認";
  ...
  const meetingDateLabel = meetingDates.length > 1
    ...
    : formatMeetingDate(meetingDates[0] ?? data.source_document.meeting_date) || "開催日未確認";
      ...
      <header className="mb-5 rounded-lg border border-[#CBD5E0] bg-white p-5 shadow-sm">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-[#718096]">
            {meetingDates.length > 0 && (data.read_quality?.unknown_date_count ?? 0) > 0 ? "確認できた開催日" : "開催期間"}: {meetingDateLabel}
          </span>
          <a
        ...
        <p className="mt-3 text-base leading-relaxed text-[#4A5568]">
          {data.source_document.title}から抽出した本文を、発言単位・質問者別に自動整理した試験版です。
          発言区切りや質問・答弁の対応は、公式会議録とあわせてご確認ください。
        </p>
      </header>

      <SourceNotice sourceDocument={data.source_document} quality={data.read_quality} sourceLabel={sourceLabel} />
