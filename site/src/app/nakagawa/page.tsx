...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "nakagawa", "members.json");
...
export default function NakagawaPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("nakagawa");
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/nakagawa/members" />
    </>
  );
}
