...
function getSession(id: string): MinutesSession | null {
  const fp = path.join(process.cwd(), "data", "fukushima", "minutes", `${id}.json`);
  try {
...
function getEnriched(id: string): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "fukushima", "minutes", "enriched", `${id}.json`);
  try {
...
export async function generateMetadata({
  ...
  const title = session
    ? `${session.name} | 福島町議会 | 北海道議会情報マップ`
    : "議事録 | 福島町議会 | 北海道議会情報マップ";
...
export async function generateStaticParams() {
  const fp = path.join(process.cwd(), "data", "fukushima", "minutes", "index.json");
  try {
...
export default async function FukushimaMinutesDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <nav className="text-sm text-[#718096] mb-5 flex items-center gap-1.5">
        <a href="/fukushima" className="hover:text-[#1B3A6B] transition-colors">福島町議会</a>
        <span aria-hidden="true">›</span>
        <a href="/fukushima/minutes" className="hover:text-[#1B3A6B] transition-colors">議事録</a>
        <span aria-hidden="true">›</span>
