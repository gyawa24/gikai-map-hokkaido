...
export const metadata: Metadata = {
  title: "福島町議会",
  description: "福島町議会の議員一覧・議事録を掲載しています。",
  openGraph: {
    title: "福島町議会 | 北海道議会情報マップ",
    description: "福島町議会の議員一覧・議事録を掲載しています。",
  },
};
...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "fukushima", "members.json");
...
export default function FukushimaMembersPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("fukushima");
    ...
    return (
      <>
        ...
        <section className="mb-6">
          ...
          <p className="text-base text-[#4A5568] leading-relaxed">福島町議会の議員情報です。</p>
        </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/fukushima/members" />
    </>
  );
}
