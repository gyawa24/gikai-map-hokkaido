...
export const metadata: Metadata = {
  title: "北斗市議会",
  description: "北斗市議会の議員一覧・議事録を掲載しています。",
  openGraph: {
    title: "北斗市議会 | 北海道議会情報マップ",
    description: "北斗市議会の議員一覧・議事録を掲載しています。",
  },
};
...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "hokuto", "members.json");
...
export default function HokutoMembersPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("hokuto");
    ...
    return (
      <>
        ...
        <section className="mb-6">
          ...
          <p className="text-base text-[#4A5568] leading-relaxed">北斗市議会の議員情報です。</p>
        </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/hokuto/members" />
    </>
  );
}
