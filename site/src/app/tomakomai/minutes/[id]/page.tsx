...
function getSession(id: string): MinutesSession | null {
  const fp = path.join(process.cwd(), "data", "tomakomai", "minutes", `${id}.json`);
  try {
...
export async function generateMetadata({
  ...
  const title = session
    ? `${session.name} | 苫小牧市議会 | 北海道議会情報マップ`
    : "議事録 | 苫小牧市議会 | 北海道議会情報マップ";
...
export default async function TomakomaiMinutesDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <nav className="text-sm text-[#718096] mb-5 flex items-center gap-1.5">
        <a href="/tomakomai" className="hover:text-[#1B3A6B] transition-colors">苫小牧市議会</a>
        <span aria-hidden="true">›</span>
        <a href="/tomakomai/minutes" className="hover:text-[#1B3A6B] transition-colors">議事録</a>
        <span aria-hidden="true">›</span>
