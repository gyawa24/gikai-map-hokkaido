...
export const metadata: Metadata = {
  title: "選挙結果 | 北海道議会情報マップ - 苫小牧市",
  description: "苫小牧市議会議員選挙の結果（得票数・当落）を掲載しています。",
};
...
function getElectionData(): ElectionData | null {
...
function partyBadgeClass(party: string | undefined): string {
  ...
  if (party.includes("自由民主") || party.includes("自民")) return "bg-blue-50 text-blue-800";
  if (party.includes("公明")) return "bg-yellow-50 text-yellow-800";
  if (party.includes("立憲") || party.includes("民主")) return "bg-green-50 text-green-800";
  if (party.includes("共産")) return "bg-red-50 text-red-800";
  if (party.includes("維新")) return "bg-orange-50 text-orange-800";
  return "bg-gray-100 text-gray-700";
}

export default function TomakomaiElectionPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <section className="mb-6">
        <h2 className="text-xl font-bold text-[#1B3A6B] mb-3">{data.election_name}</h2>
        <div className="bg-white rounded-lg border border-[#CBD5E0] p-5 flex flex-wrap gap-6">
          <div>
            ...
            <p className="text-sm text-[#1A202C] font-semibold mt-0.5">{formatDate(data.election_date)}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-[#718096]">定数</p>
            <p className="text-sm text-[#1A202C] font-semibold mt-0.5">{data.seats}名</p>
          </div>
          <div>
            <p className="text-xs font-medium text-[#718096]">立候補者数</p>
            <p className="text-sm text-[#1A202C] font-semibold mt-0.5">{data.total_candidates}名</p>
          </div>
          {data.turnout != null && (
            <div>
              <p className="text-xs font-medium text-[#718096]">投票率</p>
              <p className="text-sm text-[#1A202C] font-semibold mt-0.5">{data.turnout.toFixed(2)}%</p>
            </div>
          )}
        </div>
      </section>
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0 w-7 text-center">
                  <span className="text-sm font-semibold text-[#718096]">{i + 1}</span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-baseline gap-2">
                    <span className="text-base font-bold text-[#1A202C]">{c.name}</span>
                    <span className="text-xs text-[#718096]">{c.furigana}</span>
                    {c.status && (
                      <span className="text-xs text-[#4A5568] bg-[#F4F6F9] border border-[#E2E8F0] rounded px-1.5 py-0.5">
                        {c.status}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2 mt-1.5">
                    {c.party && (
                      <span className={`text-xs font-medium px-2 py-0.5 rounded ${partyBadgeClass(c.party)}`}>
                        {c.party}
                      </span>
                    )}
                  </div>
                  ...
                  {c.votes != null ? (
                    <>
                      <span className="text-sm font-bold text-[#1A202C]">
                        {typeof c.votes === "number" && !Number.isInteger(c.votes)
                          ? c.votes.toFixed(3)
                          : c.votes.toLocaleString()}
                      </span>
                      <span className="text-xs text-[#718096] ml-0.5">票</span>
                      {c.note && (
                        <p className="text-xs text-[#A0AEC0] mt-0.5">{c.note}</p>
                      )}
                    </>
                  ) : (
