...
function getSession(id: string): MinutesSession | null {
  const fp = path.join(process.cwd(), "data", "kushiro", "minutes", `${id}.json`);
  try {
...
function getEnriched(id: string): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "kushiro", "minutes", "enriched", `${id}.json`);
  try {
...
export async function generateStaticParams() {
  const fp = path.join(process.cwd(), "data", "kushiro", "minutes", "index.json");
  try {
...
export default async function KushiroMinutesDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <nav className="text-sm text-[#718096] mb-5 flex items-center gap-1.5">
        <a href="/kushiro" className="hover:text-[#1B3A6B] transition-colors">釧路市議会</a>
        <span aria-hidden="true">›</span>
        <a href="/kushiro/minutes" className="hover:text-[#1B3A6B] transition-colors">議事録</a>
        <span aria-hidden="true">›</span>
