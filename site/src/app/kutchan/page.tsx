...
export const metadata: Metadata = {
  title: "倶知安町議会",
  description: "倶知安町議会の議員一覧・議事録を掲載しています。",
  openGraph: {
    title: "倶知安町議会 | 北海道議会情報マップ",
    description: "倶知安町議会の議員一覧・議事録を掲載しています。",
  },
};
...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "kutchan", "members.json");
...
export default function KutchanMembersPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("kutchan");
      ...
      <>
        ...
        <div className="max-w-2xl mx-auto">
          <section className="mb-6">
            ...
            <p className="text-base text-[#4A5568] leading-relaxed">倶知安町議会の議員情報です。</p>
          </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/kutchan/members" />
    </>
  );
}
