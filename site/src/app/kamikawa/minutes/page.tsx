...
function getEnriched(councilId: number): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "kamikawa", "minutes", "enriched", `${councilId}.json`);
  try {
...
export default function KamikawaMinutesPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {items.length === 0 ? (
        ...
        <MinutesIndexClient items={items} minutesBasePath="/kamikawa/minutes" />
      )}
    </div>
  );
}
