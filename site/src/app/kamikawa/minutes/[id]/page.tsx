...
function getSession(id: string): MinutesSession | null {
  const fp = path.join(process.cwd(), "data", "kamikawa", "minutes", `${id}.json`);
  try {
...
function getEnriched(id: string): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "kamikawa", "minutes", "enriched", `${id}.json`);
  try {
...
export async function generateStaticParams() {
  const fp = path.join(process.cwd(), "data", "kamikawa", "minutes", "index.json");
  try {
...
export default async function KamikawaMinutesDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
