...
export const metadata: Metadata = {
  title: "上川町議会",
  description: "上川町議会の議員一覧・議事録を掲載しています。",
  openGraph: {
    title: "上川町議会 | 北海道議会情報マップ",
    description: "上川町議会の議員一覧・議事録を掲載しています。",
  },
};
...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "kamikawa", "members.json");
...
// v2
export default function KamikawaPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("kamikawa");
      ...
      <>
        ...
        <div className="max-w-2xl mx-auto">
          <section className="mb-6">
            ...
            <p className="text-base text-[#4A5568] leading-relaxed">上川町議会の議員情報です。</p>
          </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/kamikawa/members" />
    </>
  );
}
