...
export async function generateMetadata({
  ...
  const title = session
    ? `${session.name} | 千歳市議会 | 北海道議会情報マップ`
    : "議事録 | 千歳市議会 | 北海道議会情報マップ";
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <nav className="text-sm text-[#718096] mb-5 flex items-center gap-1.5">
        <a href="/chitose" className="hover:text-[#1B3A6B] transition-colors">千歳市議会</a>
        <span aria-hidden="true">›</span>
        <a href="/chitose/minutes" className="hover:text-[#1B3A6B] transition-colors">議事録</a>
        <span aria-hidden="true">›</span>
