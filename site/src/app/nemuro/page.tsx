...
export const metadata: Metadata = {
  title: "根室市議会",
  description: "根室市議会の議員一覧・議事録を掲載しています。",
  openGraph: {
    title: "根室市議会 | 北海道議会情報マップ",
    description: "根室市議会の議員一覧・議事録を掲載しています。",
  },
};
...
function getMembers(): Member[] {
  try {
    const fp = path.join(process.cwd(), "data", "nemuro", "members.json");
...
export default function NemuroMembersPage() {
  const members = getMembers();
  const factions = [...new Set(members.map((m) => m.faction).filter(Boolean))];
  const { count: minutesCount, latestYear } = getMinutesSummary("nemuro");
    ...
    return (
      <>
        ...
        <section className="mb-6">
          ...
          <p className="text-base text-[#4A5568] leading-relaxed">根室市議会の議員情報です。</p>
        </section>
  ...
  return (
    <>
      <CitySummaryCards
        ...
        latestYear={latestYear}
      />
      <MemberList members={members} factions={factions} activity={{}} memberHrefBase="/nemuro/members" />
    </>
  );
}
