...
function getEnriched(councilId: number): MinutesEnriched | null {
  const fp = path.join(process.cwd(), "data", "memuro", "minutes", "enriched", `${councilId}.json`);
  try {
...
// v2 - force cache bust
export default function MemuroMinutesPage() {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <section className="mb-6">
        ...
        <p className="text-base text-[#4A5568] leading-relaxed">
          芽室町議会の公式会議録です。本会議・委員会の発言内容をすべて収録しています。
          {items.length > 0 && (
      ...
      {items.length === 0 ? (
        ...
        <MinutesIndexClient items={items} minutesBasePath="/memuro/minutes" />
      )}
    </div>
  );
}
