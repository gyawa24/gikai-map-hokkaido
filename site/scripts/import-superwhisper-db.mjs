 ...
 * 使い方:
 *   # 1. yt-dlpでmp3をダウンロード
 *   yt-dlp -x --audio-format mp3 -o "tmp_audio/<id>.mp3" https://youtube.com/watch?v=XXX
 *
 *   # 2. tmp_audio/<id>.mp3 を Super Whisper にドラッグして文字起こし
 *
 *   # 3. このスクリプトを実行（Super Whisperの最新録音を自動取得）
 *   node scripts/import-superwhisper-db.mjs --id r8-teireikai1-day3-20260310
 *
 * オプション:
...
function readSuperWhisperRecordings(limit = 10) {
  const result = spawnSync("sqlite3", [
    SW_DB,
    "-json",
    `SELECT r.id, r.datetime, r.duration, r.fromFile, c.c1 AS raw_text
     FROM recording r
...
if (!sessionId) {
  console.error("Usage: node import-superwhisper-db.mjs --id <session-id> [--nth N]");
...
function save(baseDir) {
  const dest = path.join(baseDir, "chitose", "sessions", `${sessionId}.json`);
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.writeFileSync(dest, JSON.stringify(updated, null, 2));
  console.log(`  保存: ${dest}`);

  const indexPath = path.join(baseDir, "chitose", "sessions", "index.json");
...
console.log(`  git commit -m "Add transcript: ${session.title}"`);
