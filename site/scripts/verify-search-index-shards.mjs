...
const researchCoverage = readJson(path.join(GENERATED_DIR, "research-coverage.json"));
invariant(
  (researchCoverage.municipalities ?? []).every(
    (entry) => !restrictedMinutesCities.has(entry.slug)
  ),
  "restricted municipality leaked into research coverage"
);
