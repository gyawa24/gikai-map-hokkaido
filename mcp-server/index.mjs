#!/usr/bin/env node
// 地方議会.com の横断検索を Claude Code / Claude Desktop に公開する MCP サーバー。
// stdio で個人利用前提（公開API ではない）。Max Plan のトークンだけで動く。
...
const SEARCH_INDEX_PATH = path.join(SITE_DATA_DIR, "_search-index.json");
// 公開サイトでは閲覧停止している自治体（札幌など）を MCP からのみ検索可能にするための追加インデックス。
// 存在すれば search_minutes が両方をマージして返す。サイトのビルドフローからは完全に独立している。
const RESTRICTED_INDEX_PATH = path.join(__dirname, "_restricted-index.json");
const PUBLIC_BASE = "https://chihougikai.com";

// ---------------------------------------------------------------------------
// Lazy loaders — 24MB の検索インデックスは初回検索時に1回だけ読む
// ---------------------------------------------------------------------------
...
function getMunicipalities() {
  ...
  _municipalities = JSON.parse(fs.readFileSync(p, "utf-8"));
...
let _restrictedIndex = null;
let _restrictedIndexChecked = false;
function getRestrictedAgendas() {
  if (_restrictedIndexChecked) return _restrictedIndex?.agendas ?? [];
  _restrictedIndexChecked = true;
  if (!fs.existsSync(RESTRICTED_INDEX_PATH)) return [];
  try {
    _restrictedIndex = JSON.parse(fs.readFileSync(RESTRICTED_INDEX_PATH, "utf-8"));
  } catch {
    return [];
  }
  return _restrictedIndex.agendas ?? [];
}
...
// Server
// ---------------------------------------------------------------------------
const server = new McpServer({ name: "gikai", version: "0.1.0" });
...
server.tool(
  ...
  async ({ query, cities, limit }) => {
    ...
    for (const m of targets) {
      const fp = path.join(RAW_DATA_DIR, m.slug, "members.json");
...
server.tool(
  ...
  async ({ city, council_id, schedule_index, around_minute_id, max_chars }) => {
    const fp = path.join(RAW_DATA_DIR, city, "minutes", `${council_id}.json`);
    if (!fs.existsSync(fp))
...
server.tool(
  ...
  async ({ city, session_id, seg_index, include_transcript }) => {
    const fp = path.join(RAW_DATA_DIR, city, "sessions", `${session_id}.json`);
    if (!fs.existsSync(fp))
